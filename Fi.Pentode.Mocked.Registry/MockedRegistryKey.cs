...
public sealed class MockedRegistryKey : IRegistryKey
{
    ...
    public IEnumerable<string> ValueNames =>
        keys[this.path].Where(static value => values.ContainsKey(value));
    ...
    public IEnumerable<string> SubKeyNames =>
        keys[this.path].Where(static key => keys.ContainsKey(key));
    ...
    /// values".
    /// </returns>
    public static Dictionary<string, string[]> GetKeys => keys;
    ...
    /// are their values.
    /// </returns>
    public static Dictionary<string, object> GetValues => values;
